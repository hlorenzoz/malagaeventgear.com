...
| 2 | Headset microphone rental | **ready** | headset-lavalier-microphone-rental | 1 | RESOLVED 2026-07-31: business confirmed 2x Audix RAD-360 lavalier sets + 1x Audix HT5 headband mic are real (Equipamiento.csv). sound-system-rental.svx and the pillar corrected from "we don't offer" to "separate item, not bundled into packages" as a quick fix, but this is a real, distinct, previously-undocumented product - a genuine candidate for its own supporting post targeting `audiovisual-equipment-rental-service`, same reasoning as row 17 (Stage uplighting) in category 6. |
...
| # | Service | Status | Slug (existing or proposed) | Batch | Notes |
| - | ------- | ------ | ---------------------------- | ----- | ----- |
| 1 | Stage lighting rental | **ready** | stage-lighting-rental | 2 | Broadest, natural host page for this category (RGBW/specialty stage lighting/uplighting/fairy lights all real - "moving head" corrected 2026-07-31, see Iluminacion note near Summary). |
...
| 17 | Stage uplighting | **ready** | stage-uplighting | 2 | RESOLVED 2026-07-31: business confirmed the ADJ Element H6 Pack (6 fixture wireless battery uplighting kit) is real. Already given a short mention on the pillar and on lighting-ideas-for-wedding-rentals.svx (both corrected from a flat denial), but a dedicated page is a genuine candidate given it's a real, distinct, previously-undocumented product. TARGET OVERRIDE: category 6's own pillar (stage-lighting-equipment-supplier) is not yet written, so target this page at `audiovisual-equipment-rental-service` instead (siloRole: supporting) - it already partially covers this topic and needs its own supporting cluster built out. Re-target to the category 6 pillar later if/when that pillar gets written. |
...
| Audiovisual equipment rental service | audiovisual-equipment-rental-service (published 2026-07-31) | 19 | 1 | 1 | 10 | 5 | 0 | 0 | 2 (as no-grounding) |
...
- Category 2: row 2 Headset/lavalier microphone rental (new 2026-07-31, real 2x Audix RAD-360 + 1x Audix HT5 confirmed by the business, targets `audiovisual-equipment-rental-service` directly, one of the few genuine additions to that pillar's orphan supporting cluster).
- Category 5: row 4 AV system troubleshooting, row 6 Audio system calibration, row 16 AV equipment consultations, row 40 AV cable management, row 43 Hospitality AV solutions
- Category 6: row 1 Stage lighting rental (absorbs category 5 row 34 "Stage lighting installation" as a duplicate), row 17 Stage uplighting (new 2026-07-31, real ADJ Element H6 Pack wireless uplighting kit, absorbs row 18 "Wireless stage lighting" as a synonym; TARGET OVERRIDE to `audiovisual-equipment-rental-service`, see the row's own note), row 19 Stage lighting for weddings, row 20 Sound and lighting packages, row 45 Outdoor event lighting
