...
import extrasLoaders from 'virtual:blog-extras';
...
// Posts cache, computed once at module evaluation time (ADR-003, ADR-004). The date cut is
// the BUILD time (`builtAt`, pinned by scripts/vite-blog-meta.mjs), the same one the
// translations and the availability sets use, never the runtime clock: listings, sitemaps,
// routing and prerender always agree on which posts exist.
const _allPosts: BlogPost[] = buildPostsFromGlob(metaModules, new Date(builtAt));
...
// The FAQ pairs and ToC entries are NOT attached here: every listing imports this module, so
// they load per post in the post page (getPostExtras).
...
/**
 * FAQ pairs and ToC entries of ONE post, for its page only (FAQPage JSON-LD, the ToC sidebar),
 * in its own small chunk (`virtual:blog-extras/<locale>/<slug>`): English from the caches
 * (post-faqs.json, post-toc.json), a translation extracted from its body. No listing, other
 * post or other locale downloads them.
 */
export async function getPostExtras(locale: Locale, slug: string): Promise<PostExtras> {
	const posts = await extrasLoaders[locale]?.();
...
//
// A translation lives at src/content/blog/<locale>/<en-slug>.svx. Its frontmatter comes from
// `virtual:blog-translations`, one lazy chunk per locale, so a page never downloads the other
// languages. Its FAQ and ToC load per post, like English (getPostExtras). That makes the per locale API async. For 'en' it returns the English
