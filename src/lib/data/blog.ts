 ...
 * English: its own small chunk from the caches (post-faqs.json, post-toc.json, generated by
 * scripts/gen-post-faqs.ts and gen-post-toc.ts), so no other page downloads them. A translation
 * carries them already, extracted from its body in its locale's chunk.
 */
export async function getPostExtras(locale: Locale, slug: string): Promise<PostExtras> {
	if (locale !== 'en') {
		const post = await getLocalizedPost(locale, slug);
		return post ? { faqs: post.faqs, toc: post.toc } : {};
	}
	const loader = extrasLoaders[slug];
