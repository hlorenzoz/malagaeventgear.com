import type { Messages } from './en';

const t = {
	// Navigation
	nav: {
		equipment: 'Ausrüstung',
		packages: 'Pakete',
		blog: 'Blog',
		contact: 'Kontakt',
		bookNow: 'Jetzt buchen',
		blogInEnglish: 'Blog (auf Englisch)',
		language: 'Sprache',
		breadcrumbs: 'Breadcrumbs',
		brand: 'Malaga Event Gear',
		toggleTheme: 'Farbschema wechseln',
		openMenu: 'Navigationsmenü öffnen'
	},
	// Language notices (CLAUDE.md, "Idiomas soportados")
	notices: {
		// Shown on pages in a language MEG does not answer in (every locale but en and es)
		serviceLanguages: 'Wir antworten auf Englisch oder Spanisch.',
		// Top of every translated legal page
		legalTranslation:
			'Dies ist eine Übersetzung. Bei Abweichungen von der englischen Version gilt die englische Version.',
		readEnglish: 'Englische Version lesen'
	},
	// Breadcrumb names, keyed by English path segment (see i18n/breadcrumbs.ts)
	crumbs: {
		home: 'Startseite',
		packages: 'Pakete',
		blog: 'Blog',
		categories: 'Kategorien',
		category: 'Kategorie',
		author: 'Autor',
		contact: 'Kontakt',
		'about-us': 'Über uns',
		faq: 'FAQ',
		'privacy-policy': 'Datenschutzerklärung',
		'terms-of-service': 'AGB',
		'cookie-policy': 'Cookie-Richtlinie',
		gdpr: 'DSGVO',
		'meet-the-team': 'Unser Team',
		equipment: 'Ausrüstung',
		sitemap: 'Sitemap',
		'thank-you': 'Vielen Dank'
	},
	hero: {
		span: 'Für alle Arten von Veranstaltungen',
		titlePart1: 'Verleih von',
		titleGradient: 'Veranstaltungstechnik',
		titlePart2: 'in Malaga',
		subtitle:
			'Erleben Sie glasklaren Klang und beeindruckendes Licht mit unserer Premium-Technik. Perfekt für Hochzeiten, Firmenevents und exklusive Feiern an der Costa del Sol.',
		viewPricing: 'Preise ansehen',
		contactUs: 'Kontaktieren Sie uns'
	},
	bento: {
		card1Title: '#1 Reibungsloser Aufbau',
		card1Text: 'Engagierter technischer Support sorgt dafür, dass Ihre Veranstaltung von Anfang bis Ende sorgenfrei abläuft.',
		card2Title: '#2 Maßgeschneiderte Pakete',
		card2Text: 'Flexible Mietpakete, die perfekt zu jeder Veranstaltungsgröße, jedem Veranstaltungsort und jedem Budget passen.',
		card3Title: '#3 Modernste Technik',
		card3Text: 'Genießen Sie hochmoderne Veranstaltungstechnik, die die visuelle und klangliche Qualität Ihrer Produktion aufwertet.'
	},
	overview: {
		badge: 'Auf einen Blick',
		sellQ: 'Was bieten wir an?',
		sellA: 'Wir vermieten hochwertige Veranstaltungstechnik, professionelle Beschallungsanlagen, Bühnenbeleuchtung, Beamer und Leinwände, für Events in ganz Malaga und an der Costa del Sol, inklusive Lieferung, Aufbau und technischem Support vor Ort.',
		whoQ: 'Für wen ist das gedacht?',
		whoA: 'Für Paare, die eine Hochzeit planen, Unternehmen, die Konferenzen und Firmenevents veranstalten, und alle, die eine Party oder private Feier mit einwandfreiem Klang und Licht ausrichten wollen, ohne die Technik selbst zu kaufen.',
		costQ: 'Was kostet das?',
		costA: 'Festpreispakete ohne versteckte Kosten, passend zur Größe Ihrer Veranstaltung, plus individuelle Angebote für größere Produktionen.',
		costFrom: 'Ab',
		howQ: 'Wie funktioniert das?',
		howA: 'Vier einfache Schritte: Paket auswählen, Angebot anfordern, wir bestätigen und bereiten Ihre Technik vor, und unser Team liefert und baut am Tag Ihrer Veranstaltung alles auf.'
	},
	impact: {
		title: 'Unsere Zahlen',
		years: 'Jahre Erfahrung',
		clients: 'Zufriedene Kunden',
		satisfaction: 'Zufriedenheitsrate'
	},
	categories: {
		badge: 'Premium-Technik',
		title: 'Verfügbare Kategorien',
		soundTitle: 'Beschallungsanlagen',
		soundText: 'Glasklarer High-Fidelity-Sound, ideal für intime Hochzeiten oder große Firmenkonferenzen. Wir arbeiten mit führenden Marken für höchste klangliche Treue.',
		lightTitle: 'Beleuchtung',
		lightText: 'Dynamische Beleuchtungslösungen für die perfekte Atmosphäre an Ihrem Veranstaltungsort.',
		visualTitle: 'Beamer & Leinwände',
		visualText: 'Scharfe HD-Bilder für Präsentationen mit hoher visueller Wirkung.',
		fxTitle: 'Spezialeffekte & Nebelmaschinen',
		fxText: 'Schaffen Sie mit unseren professionellen Spezialeffekten und Nebelmaschinen eine beeindruckende Atmosphäre bei Ihrer Veranstaltung.',
		bookEquipment: 'Pakete buchen'
	},
	pricing: {
		badge: 'Transparente Preise',
		title: 'Maßgeschneiderte Pakete für jede Veranstaltung',
		subtitle: 'Wählen Sie aus unseren flexiblen Mietpaketen, die perfekt zu jeder Veranstaltungsgröße und jedem Budget passen. Wir machen die Planung einfach!',
		includes: 'Enthält:',
		includedServices: 'Enthaltene Leistungen:',
		optional: 'Optional:',
		check: 'Verfügbarkeit prüfen',
		mostPopular: 'Am beliebtesten',
		from: 'Ab',
		plusVat: '(zzgl. 21% MwSt.)',
		plusVatShort: '(zzgl. MwSt.)',
		bookPack: 'Buchen'
	},
	filters: {
		title: 'Filter',
		clearAll: 'Alle löschen',
		resetFilters: 'Filter zurücksetzen',
		showingResults: '{visible} von {total} Paketen werden angezeigt',
		noResults: 'Keine Pakete entsprechen Ihren Filtern. Versuchen Sie, einige Auswahlen zu entfernen!',
		openFilters: 'Filter',
		done: 'Ergebnisse anzeigen',
		purpose: 'Veranstaltungsart',
		capacity: 'Veranstaltungsgröße',
		price: 'Budget',
		equipment: 'Enthaltene Technik',
		extras: 'Optionale Extras',
		sortBy: 'Sortieren nach',
		party: 'Partys',
		wedding: 'Hochzeiten',
		corporate: 'Firmenevents',
		presentation: 'Präsentationen',
		meeting: 'Meetings',
		small: 'Klein (bis 50 Gäste)',
		medium: 'Mittel (51-80 Gäste)',
		large: 'Groß (80+ Gäste)',
		priceLow: 'Bis {price:budgetLow}',
		priceMid: '{price:budgetLow} bis {price:budgetHigh}',
		priceHigh: '{price:budgetHigh} und mehr',
		transport: 'Transport & Aufbau',
		screen: 'Leinwand / Display',
		sound: 'Beschallungsanlage',
		microphone: 'Mikrofone',
		lighting: 'Ambientebeleuchtung',
		technician: 'Live-Techniker',
		projector: 'Beamer',
		smokeMachine: 'Nebelmaschine',
		technicalAssistant: 'Technischer Assistent',
		lectern: 'Rednerpult',
		staging: 'Bühnenpodest',
		recommended: 'Empfohlen',
		priceAsc: 'Preis: aufsteigend',
		priceDesc: 'Preis: absteigend'
	},
	contact: {
		badge: 'Sofortige Antwort rund um die Uhr',
		title: 'Kontaktieren Sie uns',
		subtitle: 'Bereit, Ihre Veranstaltung aufzuwerten? Kontaktieren Sie unser technisches Team für maßgeschneiderte Angebote, Verfügbarkeitsprüfungen und fachkundige Beratung.',
		detailsTitle: 'Kontaktdaten',
		phone: 'Telefon',
		whatsapp: 'WhatsApp',
		email: 'E-Mail',
		location: 'Standort',
		hours: 'Öffnungszeiten',
		hoursText: 'Technischer Support und Logistik 24 Stunden am Tag, 7 Tage die Woche.',
		reqTitle: 'Angebot anfordern',
		formName: 'Vollständiger Name *',
		formEmail: 'E-Mail-Adresse *',
		formPhone: 'Kontakttelefon',
		formDate: 'Veranstaltungsdatum',
		formType: 'Art der Veranstaltung',
		formTypeWedding: 'Hochzeit / Feier',
		formTypeCorporate: 'Firmenveranstaltung',
		formTypeParty: 'Private Party',
		formTypeMice: 'Konferenz / MICE',
		formTypeOther: 'Andere Art von Veranstaltung',
		formMessage: 'Details zur Veranstaltung & technische Anforderungen *',
		formSubmit: 'Anfrage senden',
		formSubmitting: 'Wird gesendet...',
		formRequiredError: 'Bitte füllen Sie alle Pflichtfelder aus.',
		formErrorSubmit: 'Beim Senden Ihrer Anfrage ist etwas schiefgelaufen. Bitte versuchen Sie es erneut oder schreiben Sie uns direkt eine E-Mail.',
		formErrorTurnstile: 'Sicherheitsprüfung fehlgeschlagen. Bitte versuchen Sie es erneut.',
		formErrorRateLimited: 'Zu viele Anfragen. Bitte warten Sie einige Minuten und versuchen Sie es erneut.',
		lockedFieldNote: 'Automatisch aus einem Fehler generiert, dieses Feld kann nicht bearbeitet werden.',
		errorPrefillMessage:
			'Hallo, ich habe auf Ihrer Website eine Paketanfrage gesendet, aber die Bestätigungs-E-Mail konnte nicht zugestellt werden. Könnten Sie bitte bestätigen, dass Sie meine Anfrage erhalten haben? Referenz: {ref}',
		errorDetailsHeader: 'Übermittelte Details:',
		errorDetailSource: 'Formular-URL',
		errorDetailName: 'Name',
		errorDetailEmail: 'E-Mail',
		errorDetailPhone: 'Telefon',
		errorDetailDate: 'Veranstaltungsdatum',
		errorDetailPackage: 'Paket',
		errorDetailComments: 'Anmerkungen',
		successTitle: 'Angebot angefordert!',
		successText1: 'Hallo',
		successText2: 'wir haben Ihre Anfrage erfolgreich erhalten. Unser technisches Team in Malaga wird sie prüfen und Sie so schnell wie möglich per E-Mail kontaktieren (',
		successText3: ').',
		successButton: 'Weitere Anfrage senden',
		faqTitle: 'Häufig gestellte Fragen'
	},
	packages: {
		badge: 'Beliebte Pakete',
		title: 'Wählen Sie Ihr perfektes Paket',
		subtitle: 'Maßgeschneidert für jeden Anlass. Alle Pakete beinhalten Transport, Aufbau und technischen Support vor Ort.',
		enquire: 'Angebot anfordern'
	},
	process: {
		badge: 'So funktioniert es',
		title: 'Ihre Veranstaltung in 4 einfachen Schritten',
		s1Title: 'Paket auswählen',
		s1Desc: 'Durchstöbern Sie unsere Pakete und wählen Sie das, das zu Größe und Stil Ihrer Veranstaltung passt.',
		s2Title: 'Angebot anfordern',
		s2Desc: 'Füllen Sie unser kurzes Formular aus. Wir antworten so schnell wie möglich mit der vollständigen Verfügbarkeit.',
		s3Title: 'Bestätigen & planen',
		s3Desc: 'Unser Team bestätigt Logistik, Zugang zum Veranstaltungsort und jedes technische Detail.',
		s4Title: 'Genießen Sie Ihre Veranstaltung',
		s4Desc: 'Wir übernehmen Aufbau, Durchführung und Abbau. Null Stress für Sie.'
	},
	pricingPreview: {
		badge: 'Transparente Preise',
		title: 'Einfache Preise, alles inklusive',
		subtitle: 'Keine versteckten Kosten. Transport, Aufbau und technischer Support immer inklusive.',
		viewAll: 'Alle Pakete ansehen'
	},
	faq: {
		badge: 'FAQ',
		title: 'Häufig gestellte Fragen'
	},
	testimonials: {
		badge: 'Kundenbewertungen',
		title: 'Echte Geschichten aus echten Veranstaltungen',
		subtitle: 'Verifizierte Google-Bewertungen von Kunden von der gesamten Costa del Sol.',
		ratingLabel: 'AUSGEZEICHNET',
		basedOn: 'Basierend auf {n} Bewertungen',
		poweredBy: 'Unsere aktuellsten Bewertungen',
		readMore: 'Mehr lesen',
		readLess: 'Weniger lesen',
		seeAll: 'Alle Bewertungen ansehen',
		prevAria: 'Vorherige Bewertung',
		nextAria: 'Nächste Bewertung',
		outOfFiveStars: 'von 5 Sternen'
	},
	leadForm: {
		title: 'Sichern Sie sich Ihr Veranstaltungsdatum',
		subtitle: 'Füllen Sie das Formular aus und wir melden uns so schnell wie möglich bei Ihnen.',
		nameLabelInput: 'Vollständiger Name *',
		emailLabelInput: 'E-Mail-Adresse *',
		phoneLabelInput: 'Telefon / WhatsApp *',
		eventDateLabel: 'Veranstaltungsdatum *',
		commentsLabel: 'Fragen oder Anmerkungen',
		commentsPlaceholder: 'Erzählen Sie uns von Ihrer Veranstaltung, Veranstaltungsort, Anzahl der Gäste, besondere Anforderungen...',
		submitBtn: 'Verfügbarkeit prüfen',
		submitting: 'Wird gesendet...',
		errorRequired: 'Dieses Feld ist erforderlich.',
		errorEmail: 'Bitte geben Sie eine gültige E-Mail-Adresse ein.',
		errorPhone: 'Bitte geben Sie eine gültige Telefonnummer ein.',
		errorDateFuture: 'Das Veranstaltungsdatum muss in der Zukunft liegen.',
		errorMinLength: 'Mindestens 2 Zeichen erforderlich.',
		errorMaxLength: 'Maximal 1000 Zeichen erlaubt.',
		errorHoneypot: 'Spam erkannt.',
		noCardRequired: 'Keine Kreditkarte erforderlich, um die Verfügbarkeit zu prüfen',
		quickResponseNote: 'Antwort so schnell wie möglich',
		errorSubmit: 'Etwas ist schiefgelaufen. Bitte versuchen Sie es erneut oder kontaktieren Sie uns direkt.',
		errorTurnstile: 'Sicherheitsprüfung fehlgeschlagen. Bitte versuchen Sie es erneut.',
		errorRateLimited: 'Zu viele Anfragen. Bitte warten Sie einige Minuten und versuchen Sie es erneut.',
		emailFailTitle: 'Ihre Bestätigung konnte nicht gesendet werden',
		emailFailBody: 'Ihre Anfrage wurde gespeichert, aber unser E-Mail-System konnte sie nicht senden. Bitte kontaktieren Sie unser Team direkt, damit Ihre Anfrage nicht verloren geht.',
		emailFailAction: 'Team kontaktieren',
		emailFailDismiss: 'Schließen',
		countryCode: 'Landesvorwahl',
		responseTime: 'Wir antworten so schnell wie möglich',
		trustBadge: 'Vertraut von über 500 Veranstaltungen in Malaga'
	},
	thankYou: {
		headline: 'Vielen Dank! Ihre Anfrage ist unterwegs.',
		subheadline: 'Wir haben Ihre Anfrage erhalten und melden uns so schnell wie möglich bei Ihnen.',
		responseTime: 'Erwartete Antwortzeit: so schnell wie möglich',
		backToPackages: 'Alle Pakete durchsuchen',
		whatsappCta: 'Oder kontaktieren Sie uns jetzt über WhatsApp',
		leadLabel: 'Referenz'
	},
	gallery: {
		titleHome: 'Unsere Veranstaltungen in Aktion',
		titlePackage: 'Vergangene {pack}-Veranstaltungen'
	},
	googleMap: {
		badge: 'Standort & Google-Profil',
		title: 'Finden Sie uns bei Google',
		subtitle: 'Besuchen Sie unser offizielles Google-Unternehmensprofil oder sehen Sie unseren Standort in Malaga.',
		viewOnGoogle: 'Auf Google Maps ansehen',
		mapTitle: 'Malaga Event Gear: Google-Unternehmensprofil'
	},
	footer: {
		brandSubtitle:
			'Premium-Verleih von Beschallung, Beleuchtung und Leinwänden für exklusive Veranstaltungen in Malaga und an der Costa del Sol. Modernste Technik und maßgeschneiderter technischer Support.',
		usefulLinks: 'Nützliche Links',
		home: 'Startseite',
		packages: 'Pakete',
		blog: 'Blog',
		news: 'Neuigkeiten',
		categories: 'Kategorien',
		aboutUs: 'Über uns',
		meetTheTeam: 'Unser Team',
		contactUs: 'Kontaktieren Sie uns',
		termsOfService: 'AGB',
		privacyPolicy: 'Datenschutzerklärung',
		cookiePolicy: 'Cookie-Richtlinie',
		gdpr: 'DSGVO',
		faq: 'FAQ',
		sitemap: 'Sitemap',
		servicePackages: 'Servicepakete',
		localAddress: 'Lokale Adresse',
		listings: 'Einträge',
		onlinePresence: 'Online-Präsenz',
		moreInformation: 'Weitere Informationen',
		moreInfoText: 'Benötigen Sie mehr Details? Kontaktieren Sie uns für Infos zu unserer Veranstaltungstechnik, Preisen und Verfügbarkeit.',
		tel: 'Tel',
		clickToChat: 'Klicken zum Chatten',
		emails: 'E-Mails',
		forHire: 'Für Buchungen',
		forContact: 'Für Kontakt',
		forLegal: 'Für rechtliche Fragen',
		allRightsReserved: 'Alle Rechte vorbehalten.',
		developedBy: 'Entwickelt von',
		lorenzozTitle: 'Lorenzoz Agency: Webentwicklungsagentur und Business-Lösungen',
		mailAriaLabel: 'E-Mail senden',
		callAriaLabel: 'Malaga Event Gear anrufen: {phone}'
	},
	blog: {
		newsBadge: 'Neuigkeiten',
		byAuthor: 'Von',
		updated: 'Aktualisiert',
		shareInlineLabel: 'Teilen:',
		shareSidebarLabel: 'TEILEN',
		shareDrawerTitle: 'Diesen Beitrag teilen',
		copiedShort: 'Kopiert!',
		copiedExclaim: 'Kopiert!',
		copyShort: 'Link kopieren',
		copyLink: 'Link kopieren',
		shareOnAria: 'Auf {network} teilen',
		copyLinkAria: 'Link in die Zwischenablage kopieren',
		openSharingAria: 'Freigabeoptionen öffnen',
		closeSharingAria: 'Freigabeoptionen schließen',
		tweetLabel: 'Tweet',
		tweetAria: 'Diese Auswahl twittern',
		packagesSidebarAria: 'Seitenleiste mit Veranstaltungspaketen',
		tocSidebarAria: 'Seitenleiste mit Inhaltsverzeichnis'
	},
	whatsapp: {
		chatWithUs: 'Chatten Sie mit uns'
	},
	errorPage: {
		notFoundHeading: 'Seite nicht gefunden',
		genericHeading: 'Etwas ist schiefgelaufen',
		notFoundBody: 'Die gesuchte Seite existiert nicht mehr oder wurde verschoben. Versuchen Sie es von der Startseite aus oder kontaktieren Sie uns.',
		genericBody: 'Bei der Bearbeitung Ihrer Anfrage ist ein Problem aufgetreten. Kehren Sie zur Startseite zurück oder kontaktieren Sie uns, wir kümmern uns darum.',
		contactUs: 'Kontaktieren Sie uns',
		backHome: 'Zurück zur Startseite'
	}
} satisfies Messages;

export default t;
