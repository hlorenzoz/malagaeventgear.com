...
/**
 * Post sitemap of one locale (`/post-sitemap-de.xml`, Fase 4), mirroring
 * page-sitemap-[locale].xml. A locale with no post published has no sitemap (404), and
...
export const entries: EntryGenerator = async () => (await localesWithSitemap('post')).map((locale) => ({ locale }));

export const GET: RequestHandler = async ({ params }) => {
	const locale = params.locale as Locale;
	const [blog, map] = await Promise.all([getBlogForLocale(locale), loadContentMap(locale)]);
	const urls = localeBlogSitemapUrls('post', locale, blog, map);
