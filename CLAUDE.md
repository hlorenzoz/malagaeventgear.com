...
| Alt de las imágenes de la galería | inglés en `src/lib/data/gallery.ts` (fuente). Resto en el bloque `gallery` de `src/lib/i18n/data/<locale>.ts`, con la URL de la imagen como clave, leído con `galleryAlt()` dentro de `ImageMarquee` (así toda página y todo post que usa el carrusel sale traducido). Una imagen nueva en `gallery.ts` lleva su alt en los 12 idiomas en el mismo cambio: lo exige `localized-completeness.test.ts`, y `tests/gallery-alt.prod.spec.ts` revisa el HTML servido |
